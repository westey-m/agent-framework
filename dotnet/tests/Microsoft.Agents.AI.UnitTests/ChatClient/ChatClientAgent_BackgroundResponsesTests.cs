...
public class ChatClientAgent_BackgroundResponsesTests
{
    ...
    public async Task RunAsync_WhenContinuationTokenProvided_SkipsThreadMessagePopulationAsync()
    {
        ...
        var mockChatHistoryProvider = new Mock<AIContextProvider>();
        mockChatHistoryProvider
            .Setup(ms => ms.InvokingAsync(It.IsAny<AIContextProvider.InvokingContext>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new AIContext() { Messages = [new(ChatRole.User, "Message from chat history provider")] });
        ...
        mockChatHistoryProvider.Verify(
            ms => ms.InvokingAsync(It.IsAny<AIContextProvider.InvokingContext>(), It.IsAny<CancellationToken>()),
            Times.Never);
    ...
    public async Task RunStreamingAsync_WhenResumingStreaming_UsesUpdatesFromInitialRunForContextProviderAndChatHistoryProviderAsync()
    {
        ...
        var mockChatHistoryProvider = new Mock<AIContextProvider>();
        mockChatHistoryProvider
            .Setup(ms => ms.InvokedAsync(It.IsAny<AIContextProvider.InvokedContext>(), It.IsAny<CancellationToken>()))
            .Callback<AIContextProvider.InvokedContext, CancellationToken>((ctx, ct) => capturedMessagesAddedToStore.AddRange(ctx.ResponseMessages ?? []))
        ...
        // Assert
        mockChatHistoryProvider.Verify(ms => ms.InvokedAsync(It.IsAny<AIContextProvider.InvokedContext>(), It.IsAny<CancellationToken>()), Times.Once);
    ...
    public async Task RunStreamingAsync_WhenResumingStreaming_UsesInputMessagesFromInitialRunForContextProviderAndChatHistoryProviderAsync()
    {
        ...
        mockChatHistoryProvider
            .Setup(ms => ms.InvokedAsync(It.IsAny<AIContextProvider.InvokedContext>(), It.IsAny<CancellationToken>()))
            .Callback<AIContextProvider.InvokedContext, CancellationToken>((ctx, ct) => capturedMessagesAddedToStore.AddRange(ctx.RequestMessages))
            .Returns(new ValueTask());
