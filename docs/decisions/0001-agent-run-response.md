...
| AWS (Strands) | **Approach 3** Returns an AgentResult (Primary) with messages and a reason for the run's completion. | **Approach 2** [Streams events](https://
.com/docs/user-guide/concepts/streaming/) (Primary+Secondary) including, response text, current_tool_use, even data from "callbacks" (strands plugins) |
