...
public static class WorkflowProvider
{
    ...
    internal sealed class SetVarExecutor(FormulaSession session) : ActionExecutor(id: "set_var", session)
    {
        ...
        {
            var evaluatedValue = await context.EvaluateValueWithSensitivityAsync<object>("3").ConfigureAwait(false);
            await context.QueueStateUpdateWithSensitivityAsync(key: "TestVar", value: evaluatedValue, scopeName: "Local").ConfigureAwait(false);
