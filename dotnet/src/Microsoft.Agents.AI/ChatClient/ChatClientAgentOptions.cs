...
public sealed class ChatClientAgentOptions
{
    ...
    /// <summary>
    /// Gets or sets a factory function to create a chat history provider
    /// which will be used to provide chat history for this agent, if the underlying AI service
    /// that the agent uses does not require chat history to be stored in the service.
    /// </summary>
    public Func<AIContextProviderFactoryContext, CancellationToken, ValueTask<AIContextProvider>>? ChatHistoryProviderFactory { get; set; }
    ...
    /// <summary>
    /// Context object passed to the <see cref="ChatHistoryProviderFactory"/> to create a new instance of a chat history <see cref="AIContextProvider"/>.
    /// </summary>
    public sealed class ChatHistoryProviderFactoryContext
    {
        /// <summary>
        /// Gets or sets the serialized state of the chat history store, if any.
        /// </summary>
        /// <value><see langword="default"/> if there is no state, e.g. when the <see cref="AIContextProvider"/> is first created.</value>
        public JsonElement SerializedState { get; set; }
