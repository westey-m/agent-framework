...
- **Option 3**: Keep ChatHistoryProvider and AIContextProviders separate, but model AIContextProviders as decorators over the agent or chat client.
  ...
  - Con: Having a pipeline, assumes that we are able to modify responses as they are returned, but we have streaming and non-streaming cases. Modifying the stream of response messages either requires separate code for streaming vs non-streaming scenarios, or we need to wait until the entire stream is received, convert it to a regular AgentResponse modify it, and then convert it back to a stream. This adds complexity and/or latency. Alternatively, we could only implement a non-streaming mechanism in the decorators, but then then return messages couldn't be returned to the caller, unless we always delayed the response until the stream was fully received. If the return messages are not used, it would be confusing, since users would expect that they could modify the response messages.
