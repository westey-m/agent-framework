...
public class ChatClientAgentThreadTests
{
    ...
    public void SetChatHistoryProviderRoundtrips()
    {
        ...
        // Act
        thread.ChatHistoryProvider = historyProvider;

        // Assert
        Assert.Same(historyProvider, thread.ChatHistoryProvider);
        Assert.Null(thread.ConversationId);
    }
    ...
    public void SetChatHistoryProviderThrowsWhenConversationIdIsSet()
    {
        ...
        // Act & Assert
        var exception = Assert.Throws<InvalidOperationException>(() => thread.ChatHistoryProvider = store);
        Assert.Equal("Only the ConversationId or ChatHistoryProvider may be set, but not both and switching from one to another is not supported.", exception.Message);
    ...
    public async Task VerifyDeserializeWithMessagesAsync()
    {
        ...
        var historyProvider = thread.ChatHistoryProvider as InMemoryChatHistoryProvider;
        Assert.NotNull(historyProvider);
        Assert.Single(historyProvider);
        Assert.Equal("testAuthor", historyProvider[0].AuthorName);
    }
    ...
    {
        // Arrange
        InMemoryChatHistoryProvider store = [new(ChatRole.User, "TestContent") { AuthorName = "TestAuthor" }];
        var thread = new ChatClientAgentThread { ChatHistoryProvider = store };
    ...
    public void VerifyThreadSerializationWithCustomOptions()
    {
        ...
        var historyProviderMock = new Mock<AIContextProvider>();
        historyProviderMock
            .Setup(m => m.Serialize(options))
            .Returns(storeStateElement);
        thread.ChatHistoryProvider = historyProviderMock.Object;
    ...
    public void GetService_RequestingChatHistoryProvider_ReturnsChatHistoryProvider()
    {
        ...
        // Act
        var result = thread.GetService(typeof(InMemoryChatHistoryProvider));

        // Assert
        Assert.NotNull(result);
        Assert.Same(historyProvider, result);
    }
