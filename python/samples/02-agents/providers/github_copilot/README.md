...
| [`github_copilot_with_file_hooks.py`](github_copilot_with_file_hooks.py) | Shows how the agent treats file hooks checked into the working directory's `.github/hooks/`. They are not loaded by default; opt in with `enable_file_hooks=True` for directories you trust. |
